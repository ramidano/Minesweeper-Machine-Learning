...
class MinesweeperAI:
    ...
    def flip_ensemble(self, input_tensor):
        ...
        with torch.no_grad():
            ...
            logits_sum += torch.flip(self.model(torch.flip(input_tensor, [3])), [3])
            for k in [1, 2, 3]:
                logits_sum += torch.rot90(self.model(torch.rot90(input_tensor, k, [2, 3])), -k, [2, 3])
                logits_sum += torch.flip(torch.rot90(self.model(torch.rot90(torch.flip(input_tensor, [3]), k, [2, 3])), -k, [2, 3]), [3])
